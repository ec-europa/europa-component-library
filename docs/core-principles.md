...
- **Be framework-friendly**: ECL releases provide optimised vanilla source code which is contained within namespaces in order to allow for gradual/incremental integration with any web framework. ECL does not provide global styles which do not cause conflicts with existing applications' styles and scripts.
- **Be publicly-accessible**: releases are well-documented steps of incremental improvements and are published on several repositories at the same time in order to allow implementers to choose their preferred way of consuming ECL.
...
- **Scalability**: building blocks (components) of ECL are self-contained, independently-tested and maintained pieces of code.
