...
Twig templates have been updated to deliver the new markup for the icon, so this is mostly transparent, execpt for a few new parameters:
...
- Twitter and X now are 2 different icons. So whenever you ware using `twitter` icon, it has to be changed to `x`
