...
document.addEventListener(
  ...
  (event) => {
    if (event.target.tagName === 'DETAILS') {
      detailsState[event.target.id] = event.target.open;
    }
  },
  true,
);
...
export const Default = (args) => {
  const wrapperClass = args.color_mode
    ? `ecl-color-mode--${args.color_mode} ecl-u-bg-surface-0 ecl-u-type-color-on-surface-swap-0 ecl-u-pa-m`
    : '';
  ...
  return `
    ...
    <details id="list" class="ecl-content-color" ${detailsState.list ? 'open' : ''}>
