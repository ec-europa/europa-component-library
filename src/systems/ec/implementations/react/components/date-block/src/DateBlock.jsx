...
const DateBlock = ({
  ...
  if (monthFull !== '' && monthFull !== month) {
    monthMarkup = (
      <abbr aria-hidden="true" title={monthFull} className="ecl-date-block__month">
        {month}
      </abbr>
    );
  } else {
  ...
  return (
    <time {...props} className={classNames} dateTime={dateTime}>
      ...
      <span aria-hidden="true" className="ecl-date-block__day">{day}</span>
      {monthMarkup}
      <span aria-hidden="true" className="ecl-date-block__year">{year}</span>
    </time>
  );
};
