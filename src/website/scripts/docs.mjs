...
const outputDirs = [
  path.resolve(publicDir, apisDir, 'ec'),
  path.resolve(publicDir, apisDir, 'eu'),
];
...
outputDirs.forEach((outputDir) => {
  ...
  // Initialize an empty string to store concatenated documentation content
  ...
  files.forEach((file) => {
    ...
    try {
      ...
      if (content) {
        ...
        content = content.replace('<a href="index.html">Home</a>', '<a href="/apis/ec/index.html">Home</a>');

        fs.writeFileSync(outputFile, content, 'utf-8');
        // Concatenate the content of each documentation file
