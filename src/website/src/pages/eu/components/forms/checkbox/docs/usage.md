...
order: 1
---
The Checkbox enables users to make independent, non-exclusive selections from a predefined set of options. Unlike the [Radio](https://ec.europa.eu/component-library/eu/components/forms/radio/code/) component that enforces a single selection, the Checkbox is designed for situations where multiple answers may simultaneously be valid or correct.
...
- do not use for mutually exclusive items (i.e. only one option can be correct or active at a time), use the [Radio](https://ec.europa.eu/component-library/eu/components/forms/radio/code/) component instead
- do not use for long or dynamic option lists (i.e. large number or varied input, based on context) - a [Multiple Select](https://ec.europa.eu/component-library/eu/components/forms/select/code/) dropdown is more space-efficient and easier to navigate, particularly on smaller screens
