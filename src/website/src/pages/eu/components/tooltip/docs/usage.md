...
- do not use tooltips to display long-form content, instructions, or multiple items. If the content is substantive, consider a [Popover](https://ec.europa.eu/component-library/eu/components/popover/code/)
- do not use as the sole mechanism for conveying status, error or warning information. Consider instead a [Notification](https://ec.europa.eu/component-library/eu/components/notification/code/), so the information is reliably visible without requiring user interaction
