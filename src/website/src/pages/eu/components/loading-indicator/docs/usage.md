...
- do not use when an error or empty state should be shown (i.e. if a request has completed but returned no results or has failed)
  ...
  - display an appropriate error message or [Notifications](https://ec.europa.eu/component-library/eu/components/notification/code/) rather than leaving the indicator in place
...
- do not use when the context requires a blocking, decision-oriented pause (i.e. a process requires the user to wait and then actively confirm an outcome)
  ...
  - a [Modal](https://ec.europa.eu/component-library/eu/components/modal/code/) with appropriate messaging is more appropriate than a loading indicator.
