...
The detection is based on the standard HTML attribute `dir`. When using a right to left language, make sur to set `dir="rtl"` anywhere at upper level on the page (it could be the html or body tag for instance).

If not done, some elements may be misplaced when using right to left display.
