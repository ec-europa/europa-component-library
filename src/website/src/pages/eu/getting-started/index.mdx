...
<html lang="en" class="no-js">
  <head>
    ...
    <!-- Preload Duet Date Picker scripts to improve loading performance -->
    <link
      rel="preload"
    ...
    <!-- Only needed if an ECL datepicker is present in the page -->
    <script
      type="module"
...
There are also other optional css files distributed that can be used, duet js is added because it's needed for the ECL datepicker, see below for further information.
...
Download a `preset-eu` package from the release page to see all the files available so to know how to reach them in the CDN, the file system is the same.

For more security, use the SRI hashes corresponding to the files when using them from the CDN.
They can be found in the [release page](https://github.com/eu-europa/europa-component-library/releases), in a json file, but here are the most common ones:

<details>
  <summary class="ecl-u-type-s ecl-u-mt-s">SRI hashes (click to expand)</summary>
...
When using the CDN for logos be aware that it's not possible to serve svgs from an external domain without using some workaround, see the "Additional information" below.
...
For a number of languages, content is displayed right to left. ECL components are designed to take this into consideration, but need some information to know what direction to use.

The detection is based on the standard HTML attribute `dir`. When using a right to left language, make sure to set `dir="rtl"` anywhere at upper level on the page (it could be the html or body tag for instance).

If not done, some elements may be misplaced when using right to left display; the default display remains left to right.
...
Icons are provided by Webtools, and may require additional configuration or extra files.

Please check documentation on Webtools site for more information:
...
Therefore **duet js needs to be loaded or bundled by the application or website using ECL**, depending on the needs, the library is only required when a datepicker is instantiated in a page.
...
- all useful information concerning releases content can be found on the [release page](https://github.com/eu-europa/europa-component-library/releases)
...
ECL releases npm packages for:
  ...
  <ul>
    ...
    <li>components (css, js, twig) `@ecl/{name}`</li>
