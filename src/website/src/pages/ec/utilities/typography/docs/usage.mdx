...
Use classes `ecl-u-type-heading-1` to `ecl-u-type-heading-6` to apply headings styles.

An additional very large size is also available with `ecl-u-type-display`.
...
Use classes `ecl-u-type-microcopy-m` to `ecl-u-type-microcopy-2xs` to apply microcopy styles.
...
- all <Link to="/ec/guidelines/colours/">EC standard colors</Link> (main and status)
- all "on-surface" colors from color modes
- black and white
...
By using classes `ecl-u-type-highlight` you can put emphasis on the text.

If you are using a specific color mode, highlight color will automatically adapt.

## Text enhance

By using classes `ecl-u-type-enhance`, `ecl-u-type-enhance-strong` or `ecl-u-type-enhance-light` you can display a block of text in a prominent manner.
