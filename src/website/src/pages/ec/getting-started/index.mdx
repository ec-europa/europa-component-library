...
- make sure that you are correctly loading the icons and the logo by using the right paths. You will usually find these assets under the `/images` folder of the preset you're using. We advise you to host the SVG sprites on the same domain as your website in order to avoid the `Unsafe attempt to load URL`, CORS kind of errors. If you still want to use the SVG sprite from the CDN, you can use a general workaround [svg4everybody](https://github.com/jonathantneal/svg4everybody) and initialize it with: `svg4everybody({ polyfill: true });` this will fix the rendering of your icons or alternatively, **ECL has its own solution to inline the svgs to make them work also in case you use the file from an external domain**. Just set the `cors` parameter to true if you are using the icon template or add the needed `data-ecl-auto-init="Icon"` to your icon markup to get this functionality in place using the provided js.
