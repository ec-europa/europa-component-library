...
Buttons use 3 major attributes:

- Labels
  - Should be clear and distinct.
  - Use verbs to describe the action performed when interacting with the button.
  - Keep it short, no more than 3 words and only capitalise the first word.
- Colour
  - Choose a colour that provides sufficient contrast between the label and background.
- Icon
...
To trigger an action. Actions _ which affect the website's state are buttons. _ Actions include: Sign up, Login, Register, Search, Submit... etc
...
For navigational purposes. If the action triggered sends a user to a different page, opens up a page in a new tab or jump to another section within the current page, use links instead because these actions don’t change anything on the website’s state.
