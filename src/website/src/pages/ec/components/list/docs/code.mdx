---
title: Showcase
order: 2
---

import Playground from '@ecl/website-components/Playground';
import { List } from '@ecl/ec-react-component-list';

import demoContentText from '@ecl/ec-specs-list/demo/data--text';
import demoContentLink from '@ecl/ec-specs-list/demo/data--link';
import demoContentBlock from '@ecl/ec-specs-list/demo/data--block';
import demoContentNavigationBlock from '@ecl/ec-specs-list/demo/data--navigation-block';
import demoContentNavigationLink from '@ecl/ec-specs-list/demo/data--navigation-link';

## Unordered list

Content could be links or simple text

<Playground system="ec" selectedKind="List" selectedStory="standard">
  <List
    {...demoContentText}
    hasBullet={true}
    isOrdered={false}
    hasSeparator={false}
  />
</Playground>

## Ordered list

Content could be links or simple text

<Playground system="ec" selectedKind="List" selectedStory="standard">
  <List
    {...demoContentLink}
    hasBullet={true}
    isOrdered={true}
    hasSeparator={false}
  />

<List
  {...demoContentLink}
  hasBullet={true}
  isOrdered={true}
  orderedStyle="call"
  hasSeparator={false}
/>

  <List
    {...demoContentLink}
    hasBullet={true}
    isOrdered={true}
    orderedStyle="primary"
    hasSeparator={false}
  />
</Playground>

## List with separator

<Playground system="ec" selectedKind="List" selectedStory="standard">
  <List
    {...demoContentLink}
    hasBullet={false}
    isOrdered={false}
    hasSeparator={true}
  />
</Playground>

## List with block of text

<Playground system="ec" selectedKind="List" selectedStory="text block">
  <List
    {...demoContentBlock}
    hasBullet={false}
    isOrdered={false}
    hasSeparator={false}
  />
</Playground>

## Navigation list with block of text

<Playground system="ec" selectedKind="List" selectedStory="navigation block">
  <List
    {...demoContentNavigationBlock}
    hasBullet={false}
    isOrdered={false}
    hasSeparator={false}
  />
</Playground>

## Navigation list with links

<Playground system="ec" selectedKind="List" selectedStory="navigation links">
  <List
    {...demoContentNavigationLink}
    hasBullet={false}
    isOrdered={false}
    hasSeparator={false}
  />
</Playground>
